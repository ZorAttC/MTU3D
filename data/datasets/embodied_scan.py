...
@DATASET_REGISTRY.register()
class EmbodiedScanInstSegSPconv(EmbodiedScanBase):
    def __init__(self, cfg, dataset_name, split):
        if split == 'test':
            split = 'val'
        super().__init__(cfg, dataset_name, split)
    ...
    def get_scene(self, scan_id, sub_frame_id):
        ...
        seg_point_count = scatter_add(torch.ones_like(point2seg_id), point2seg_id, dim=0)
         ...
         # voxelize coorinates, features and labels
        transform = sonata.transform.my_pipeline()
        coordinates_np = coordinates.clone().detach().cpu().numpy()
        # import pdb; pdb.set_trace()
        point={
            'coord': coordinates_np,
            'color': color.astype(np.float32),
            'normal': np.zeros_like(coordinates_np, dtype=np.float32)
        }
        sonata_points = transform(point)
        voxel_coordinates = sonata_points['grid_coord']
        unique_map = sonata_points['unique_map']
        # _, unique_map, inverse_map = ME.utils.sparse_quantize(coordinates=voxel_coordinates, return_index=True, return_inverse=True)
        voxel_features = features[unique_map]
        voxel2seg_id = point2seg_id[unique_map]
        inverse_map = sonata_points['inverse']
        assert len(sonata_points['grid_coord']) == len(voxel_coordinates)
...
@DATASET_REGISTRY.register()
class EmbodiedScanInstSegScanNetSPconv(EmbodiedScanInstSegSPconv):
