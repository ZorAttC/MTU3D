...
# p4: p 代表 "pixel"（像素）或 "point"（点），在这里可以理解为空间尺度或分辨率的级别。p4 意味着这一层操作的是一个已经被下采样过的特征图，其分辨率是原始输入的 1/4。

# p1 通常指原始分辨率。
# p2 指分辨率为原始的 1/2。
# p4 指分辨率为原始的 1/4。
# p8 指分辨率为原始的 1/8。
# s2: s 代表 "stride"（步长）。s2 意味着这个卷积操作的步长是 2。

# 在 MinkUNet 的上下文中，一个步长为 2 的卷积 (s2) 会导致稀疏张量的空间分辨率减半。

# 所以，在变量名 conv3p2s2 中：

# conv3: 表示这是第三个主要的卷积下采样层。
# p2: 表示这一层的输入特征图的分辨率是 p2（原始的 1/2）。
# s2: 表示这个卷积操作的步长是 2。
    def forward(self, x):
